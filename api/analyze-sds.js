...
- LITHIUM BATTERY MANUFACTURER CHECK (only for lithium battery/cell docs — UN3480 / UN3481 / UN3090 / UN3091): Extract the cell/battery manufacturer (maker) from the document (usually Section 1 product/company identifier, the brand, or the UN 38.3 test applicant). The carrier's operations team (운항팀) approves ONLY these makers for DG lithium batteries: **SAMSUNG SDI, LG ENERGY SOLUTION (incl. LG Chem, and its joint-venture / production entity "PT. HLI GREEN POWER" — a Hyundai Glovis × LG Energy Solution JV whose actual cell maker is LG Energy Solution, so treat PT. HLI GREEN POWER / HLI GREEN POWER as APPROVED), SK ON**. Match by the manufacturer's actual company name; do NOT approve a different company merely because the document text mentions an approved brand elsewhere.
  * ⚠️ The manufacturer restriction applies ONLY when the battery is shipped AS DANGEROUS GOODS (DG). ONLY if dg_status is "NON_DG" (e.g., excepted under SP188 — lithium ion cell <=20Wh / battery <=100Wh, etc.), there is NO manufacturer restriction at all -> set manufacturer_status = "N/A" regardless of the maker (still extract the manufacturer name for reference if visible), and in basis (Korean) note "SP188 비위험물(NON-DG)로 분류되어 제조사 제한이 적용되지 않습니다". For dg_status "DG" OR "UNCLEAR" (e.g., a UN 38.3 TEST REPORT page where the maker is shown but Wh/UN are not on that page), STILL perform the APPROVED / NOT_APPROVED / UNKNOWN maker checks below against the approved list (do NOT set N/A) — because if shipped as DG the maker matters.
