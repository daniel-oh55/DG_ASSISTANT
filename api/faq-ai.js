...
- **⚠️ 혼적·격리 코드를 묻는 질문인데 위 [IMDG 격리표 판정 결과]가 '해당 없음'으로 비어 있으면(시스템이 UN번호로 표를 조회하지 못한 경우), 절대 격리코드를 추정·단정하지 마세요. 대신 "정확한 격리코드는 UN번호로 좌측 [격리규정 확인] 메뉴에서 조회하시거나 UN번호를 명확히(예: UN1993) 입력해 다시 문의해 주세요"라고 안내하세요. (틀린 코드를 말하는 것보다 조회를 안내하는 것이 안전합니다.)**
