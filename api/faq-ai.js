...
→ 전체 혼적 결론: 'SKR/HAL 사내 규정에 따라' 혼적 불가(분리 선적). IMDG 일반 격리표가 '같은 컨테이너 적재 가능(X)'이어도 이 RFDG 규정이 우선합니다. (예외: 해당 리튬배터리가 SP188로 비위험물(NON-DG)로 분류되는 경우에 한해 DRY 혼적 가능)
...
- **⚠️ 위에 [SKR/HAL RFDG 혼적 금지] 블록이 있으면 그 결론(혼적 불가·분리 선적)을 IMDG 일반 격리표보다 절대적으로 우선하세요.** IMDG 표가 '같은 컨테이너 가능(X)'이라도, SKR/HAL 사내 규정상 해당 리튬배터리는 별도 RFDG 컨테이너로 분리하고 나머지 DRY DG 화물만 따로 혼적하는 것이 최종 결론입니다. (일반 규칙: 리튬이온 배터리 UN3480·3481을 DG로 선적하면 RFDG 필수 → 다른 UNNO 화물과 혼적 불가) **이 RFDG 혼적 금지는 IMDG 국제규정이 아니라 SKR/HAL 사내 규정이므로, 회신에 "SKR/HAL 사내 규정에 따라"처럼 출처를 분명히 밝혀 주세요.**
