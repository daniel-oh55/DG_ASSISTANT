...
${rfdgUnsTxt}(리튬이온 배터리)을 위험물(DG)로 선적하면 RFDG(Reefer) 컨테이너가 필수이며, RFDG 화물은 같은 UNNO가 아닌 다른 화물과 같은 컨테이너 혼적이 금지됩니다(단 UN3480·UN3481끼리는 허가). 따라서:
- ${rfdgUnsTxt} → 별도 RFDG 컨테이너에 단독 선적.
- ${rfdgDryTxt} → 그들끼리만 별도 DRY DG 컨테이너에 IMDG 격리표 기준으로 혼적.
→ 전체 혼적 결론: 자사 규정상 '혼적 불가(분리 선적)'. IMDG 일반 격리표가 '같은 컨테이너 적재 가능(X)'이어도 이 RFDG 규정이 우선합니다. (예외: 해당 리튬배터리가 SP188로 비위험물(NON-DG)로 분류되는 경우에 한해 DRY 혼적 가능)
...
- **⚠️ 위에 [SKR/HAL RFDG 혼적 금지] 블록이 있으면 그 결론(혼적 불가·분리 선적)을 IMDG 일반 격리표보다 절대적으로 우선하세요.** IMDG 표가 '같은 컨테이너 가능(X)'이라도, 자사 RFDG 규정상 해당 리튬배터리는 별도 RFDG 컨테이너로 분리하고 나머지 DRY DG 화물만 따로 혼적하는 것이 최종 결론입니다. (일반 규칙: 리튬이온 배터리 UN3480·3481을 DG로 선적하면 RFDG 필수 → 다른 UNNO 화물과 혼적 불가)
...
- **⚠️ 위에 [SKR/HAL RFDG 혼적 금지] 블록이 있으면 그 결론(혼적 불가·분리 선적)을 IMDG 일반 격리표보다 절대적으로 우선하세요.** IMDG 표가 '같은 컨테이너 가능(X)'이라도, 자사 RFDG 규정상 해당 리튬배터리(UN3480·3481)는 별도 RFDG 컨테이너에 단독 선적하고 나머지 DRY DG 화물만 따로 혼적하는 것이 최종 결론입니다. 일반 규칙으로도, 리튬이온 배터리(UN3480·3481)를 위험물(DG)로 선적하면 자사 규정상 RFDG(Reefer)가 필수이고 RFDG 화물은 같은 UNNO가 아닌 다른 화물과 혼적할 수 없습니다(단 UN3480·3481끼리는 허가, SP188 비위험물 예외).
